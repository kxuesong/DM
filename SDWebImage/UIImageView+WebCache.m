...
- (void)sd_setimagewithString:(NSString *)string
{
    CGFloat height = CGRectGetHeight(self.frame);
    self.layer.cornerRadius = height/2.0;
    self.layer.masksToBounds = YES;
    NSURL *url = [NSURL URLWithString:string];
    [self sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"3_0000_图层-3"] options:0 progress:nil completed:nil];
}

- (CGFloat)sd_setimagewithString:(NSString *)string placeholderImage:(UIImage *)placeholder {
    NSString *urlString = [NSString stringWithFormat:@"http://210.44.64.48%@",string];
    NSURL *url = [NSURL URLWithString:urlString];
    [self sd_setImageWithURL:url placeholderImage:placeholder options:0 progress:nil completed:nil];
    CGSize size = [UIScreen mainScreen].bounds.size;
    return placeholder.size.height*size.width/placeholder.size.width;
}
